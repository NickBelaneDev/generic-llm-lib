...
class MCPClientWrapper:
    ...
    async def load_into(self, registry: ToolRegistry):
    ...
    def _register_single_tool(self, registry: ToolRegistry, tool: MCPTool):
        ...
        async def mcp_proxy(**kwargs) -> Any:
            if not self._session:
            ...
            for c in mcp_result.content:
                if c.type == "text":
                    ...
                    output.append(f"[Image: {c.mimeType}]")
                elif c.type == "resource":
                    output.append(f"[Resource: {c.uri}]")
                else:
                    output.append(f"[Unknown content type: {c.type}]")

            result_text = "\n".join(output)
            logger.debug("Tool '%s' result: %s", tool_name, result_text[:200] + "..." if len(result_text) > 200 else result_text)
            return result_text
