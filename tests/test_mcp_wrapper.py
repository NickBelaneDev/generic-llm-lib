...
@pytest.mark.asyncio
async def test_mcp_wrapper_lifecycle(mock_session):
...
@pytest.mark.asyncio
async def test_mcp_proxy_execution(mock_registry, mock_session):
    """Test that the registered proxy function calls the MCP session."""
    
    # Mock MCP tool
    tools_result = ListToolsResult(
        tools=[MCPTool(name="test_tool", description="desc", inputSchema={})]
    )
    mock_session.list_tools = AsyncMock(return_value=tools_result)
    
    # Mock execution result
    mock_session.call_tool = AsyncMock(return_value=CallToolResult(
        content=[TextContent(type="text", text="Result from MCP")]
    ))
