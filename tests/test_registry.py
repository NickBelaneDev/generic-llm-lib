...
from generic_llm_lib import ToolRegistry, GeminiToolRegistry, OpenAIToolRegistry, ToolDefinition
...
def test_gemini_registry_sanitizes_invalid_required_property() -> None:
    """
    Tests that the GeminiToolRegistry correctly sanitizes a schema
    where a 'required' property is not defined in 'properties'.
    """
    registry = GeminiToolRegistry()

    # Manually create a faulty schema
    invalid_schema = {
        "type": "object",
        "properties": {"defined_prop": {"type": "string", "description": "A defined property"}},
    ...
    # Generate the tool object for the Gemini API
    tool_obj = registry.tool_object
    assert tool_obj is not None

    # Verify that the 'required' list was sanitized
    declaration = tool_obj.function_declarations[0]
    sanitized_params = declaration.parameters

    # The sanitized 'required' list should only contain 'defined_prop'
    assert hasattr(sanitized_params, "required")
    assert sanitized_params.required == ["defined_prop"]
    assert "undefined_prop" not in sanitized_params.required
